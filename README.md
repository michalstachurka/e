...
Carport używa parametrycznej blachy trapezowej z osobną, obowiązkową warstwą antykondensacyjną od spodu. Screen ZIP i roleta zewnętrzna są domyślnie osadzane w renderowanych wnękach okiennych; konfigurator pozwala zestawić do 8 osobnych rolet obok siebie. Wartość `unitCount` jest częścią wersjonowanej konfiguracji, a starsze zapisy bez tego pola są odczytywane jako jedna roleta.

Konfigurator ma osiem typów produktów. Żaluzja fasadowa ma własny renderer: pola okienne, montaż natynkowy/we wnęce/podtynkowy, demonstracyjne lamele C80/Z90, prowadnice albo linki, niezależny stopień opuszczenia i kąt lameli, napęd oraz automatykę pogodową. Pergola, weranda i carport korzystają ze wspólnego, wersjonowanego modułu shutters aluminiowych. Panel może być stały lub przesuwny, a poziome albo pionowe lamele — stałe lub regulowane. Stage 1 generuje do 4 paneli na bok; jest to limit geometrii, nie przyszłego planu SaaS.

Garaż blaszany ma osobny renderer parametryczny: dach jedno- lub dwuspadowy, proceduralne przetłoczenia blachy, 1–2 bramy, 0–4 okna, drzwi, wiatę boczną, orynnowanie, kotwienie, napęd i opcjonalny filc antykondensacyjny. Limity liczby otworów są częścią wersjonowanej definicji pilota, a nie przyszłego planu SaaS. Wymiary, przekroje, kompatybilność, wyposażenie, ceny i BOM wszystkich nowych produktów i akcesoriów pozostają danymi `demoOnly` do zatwierdzenia przez producenta.
